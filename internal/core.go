...
// In empirical tests, the maximum message size that can cross the core boundary is ~64MB.
...
func (c *CoreWrapper) Invoke(ctx context.Context, invokeConfig InvokeConfig) (*string, error) {
	...
	if len(input) > messageLimit {
		return nil, fmt.Errorf("message size exceeds the limit of %d bytes", messageLimit)
	}
...
type InnerClient struct {
	// ponytail: one lock per SDK client; split read/refresh locks if desktop throughput matters.
	Mu     sync.Mutex
	ID     uint64
	Config ClientConfig
	Core   CoreWrapper
